...
export default class ScrollViewport extends Component {
  ...
  scrolled = () => {
    this.setState({ offset: Math.max(0, this.base && -this.base.getBoundingClientRect().top || 0) });
    if (this.props.sync) this.forceUpdate();
  };

  computeRowHeight() {
    if (this._height) return this._height;
    let el = this.base && this.base.firstElementChild && this.base.firstElementChild.firstElementChild;
    return this._height = (el && el.offsetHeight || 0);
  }

  componentDidUpdate() { this.resized(); }

  componentDidMount() {
    this.resized();
    this.scrolled();
    addEventListener('resize', this.resized, { passive: true });
    addEventListener('scroll', this.scrolled, { passive: true });
  }

  componentWillUnmount() {
    removeEventListener('resize', this.resized, { passive: true });
    removeEventListener('scroll', this.scrolled, { passive: true });
  }
  ...
  render({ items, renderer, overscan = 10, rowHeight, defaultRowHeight, internalClass, ...props }, { offset = 0, height = 0 }) {
    ...
    (props.style || (props.style = {})).height = rowHeight * items.length + 'px';
    ...
    return (
      <div {...props}>
        <div class={internalClass} style={{ position: 'relative', top: start * rowHeight }}>
          {renderer(items.slice(start, start + 1 + visibleRowCount))}
        </div>
      </div>
    );
  }
